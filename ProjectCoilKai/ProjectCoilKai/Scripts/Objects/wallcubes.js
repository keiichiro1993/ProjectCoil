function WallCubes() {
    ...
    this.createMesh = function () {
        ...
        for (var i = 0; i < 180; i++) {
            ...
            m = r2;
            this.cube[i] = new THREE.Mesh(cubeGeometry[g], cubeMaterial[m]);

            this.cube[i].position = { x: nrn[i * 3], y: nrn[i * 3 + 1], z: nrn[i * 3 + 2] };
            if (g == 0) {
                this.cube[i].position.x += 1;
                this.cube[i].position.y += 1;
                this.cube[i].position.z += 1;
            } else {
            }
